...
common_neighbours_count_range = range(0, max_common_neighbours_possible + 1)
...
def embeddedness_vs_individual_traids_over_time(args):
    ...
    for year in data.all_years:
        ...
        print "t0=%s" % str([fraction(t0, t0 + t1p + t1n + t2p + t2n + t3, 0)
                             for (t0, (t1p, t1n), (t2p, t2n), t3) in traid_counts]).replace(",", " ")
        print "t1p=%s" % str([fraction(t1p, t0 + t1p + t1n + t2p + t2n + t3, 0)
                              for (t0, (t1p, t1n), (t2p, t2n), t3) in traid_counts]).replace(",", " ")
        print "t1n=%s" % str([fraction(t1n, t0 + t1p + t1n + t2p + t2n + t3, 0)
                              for (t0, (t1p, t1n), (t2p, t2n), t3) in traid_counts]).replace(",", " ")
        print "t2p=%s" % str([fraction(t2p, t0 + t1p + t1n + t2p + t2n + t3, 0)
                              for (t0, (t1p, t1n), (t2p, t2n), t3) in traid_counts]).replace(",", " ")
        print "t2n=%s" % str([fraction(t2n, t0 + t1p + t1n + t2p + t2n + t3, 0)
                              for (t0, (t1p, t1n), (t2p, t2n), t3) in traid_counts]).replace(",", " ")
        print "t3=%s" % str([fraction(t3, t0 + t1p + t1n + t2p + t2n + t3, 0)
                             for (t0, (t1p, t1n), (t2p, t2n), t3) in traid_counts]).replace(",", " ")
        print "plot(x,t0,'y-*',x,t1p,'m-*',x,t1n,'r-o',x,t2p,'g-*',x,t2n,'b-o',x,t3,'k-*');"
        print "xlabel('Number of common neighbours');"
        print "ylabel('Percentage of triangles');"
        print "legend('T0','T1+','T1-','T2+','T2-','T3');"
